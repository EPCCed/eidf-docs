...
At present, the TRE GPU cluster only provides one type of GPU:`[NVIDIA A100-SXM4-40GB]`.
...
For future compatibility (if additional GPU types become available), you can make the GPU resource request more specific by adding a node selector to your pod template:
...
1. Open the command line on your TRE VM with access to the TRE GPU Cluster.
