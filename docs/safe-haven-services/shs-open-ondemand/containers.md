...
Some Open OnDemand apps allow you to run containers, which package up software, services and their dependencies. Some of the apps provided by Open OnDemand run containers built for the SHS Open OnDemand service (e.g., JupyterLab and RStudio Server). Other apps allow you to run your own project-specific containers that you have been authorised to run within your safe haven.
...
Open OnDemand uses the SHS Container Execution Service tools to run containers. Consequently, containers run via Open OnDemand **must** conform to the requirements of the SHS Container Execution Service. See the [SHS Container User Guide](../shs-container-user-guide/introduction.md) for details of these requirements.

---

## Container registries

The container registries supported by the SHS Container Execution Service, and so supported by Open OnDemand and its apps, are as follows:
...
!!! Note
    ...
    For ECDF GitLab, do not put the port number, 5050, into the URL. The SHS Container Execution Service tools will automatically insert this into ECDF GitLab URLs.

---

## Sharing files between a back-end and a container

When a container is run via the SHS Container Execution Service, three directories on the back-end are always mounted into the container:
...
!!! Note
    ...
    Your project data files, in a project-specific directory under `/safe_data` are **not** available on SHS-level back-ends (e.g., the Superdome Flex). For these, you will need to stage your data to the SHS-level back-end following your project- and safe haven-specific processes for the use of SHS-level services.
