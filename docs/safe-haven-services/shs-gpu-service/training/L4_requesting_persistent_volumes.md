...
``` bash
uid=0001(<shs_username>) gid=0001(<shs_username>) groups=0001(<shs_username>),0002(<project_id>),…
```
...
**User Output PVC:**
`claimName: "pvc-<safe_heaven>-<project_id>-users-<first part of shs_username before _ >"`
Mounted read-write at /safe_outputs. Dedicated to the individual user identified by their FreeIPA username `<shs_username>`.
...
!!! note "Example mapping for clarity"
    ...
    - `<shs_username>` = `test_nsh-2024-0000`
...
Your FreeIPA username `<shs_username>` corresponds to the user you log in as on the Desktop VM.
...
spec:
  ...
  template:
    ...
    spec:
      securityContext:
        runAsUser: <shs_user_id>
        runAsGroup: <shs_usergroup_id>
        fsGroup: <shs_usergroup_id>
      containers:
        - name: cudasample
