...
When the Job status updates to 'Running', a **Host** link will appear on the job card, which allows you to log in to the back-end on which the job, and so the RStudion Server container, is now running. A 'RStudio Server running in container CONTAINER_NAME' message will appear along with a **Connect to RStudio Server** button. The RStudio Server container is now ready for use.
