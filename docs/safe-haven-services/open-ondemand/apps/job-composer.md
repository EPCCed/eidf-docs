...
1. Update the script as follows:
    ...
    * The script also creates a file, `arg_file.txt`, with container-specific arguments to be passed directly to the container when it is run. The `epcc-ces-hello` container will doze for `-d` seconds, then issue a greeting to the name cited in `-n` (here, the current user).
